...
def _connector(provider: ProviderType, *, connector_id: str = ""):
    ...
    return connector_for(provider, connector_id=connector_id)
...
async def list_capabilities(provider: ProviderType = ProviderType.SIMULATOR, connector_id: str = "") -> dict[str, Any]:
    _feature_enabled()
    try:
        manifest = _connector(provider, connector_id=connector_id).list_capabilities()
...
async def list_connector_catalog() -> dict[str, Any]:
    """Ticketing and monitoring connector types this service can actually connect to,
    each tagged with the discovery_scope fields its connection form must collect."""
    return {"connectors": [
        {"connector_id": "jira", "provider_type": "itsm", "display_name": "Jira",
         "fields": [{"name": "base_url", "label": "Site URL", "placeholder": "https://acme.atlassian.net"},
                    {"name": "email", "label": "Account email"}],
         "secret_label": "API token"},
        {"connector_id": "servicenow", "provider_type": "itsm", "display_name": "ServiceNow",
         "fields": [{"name": "base_url", "label": "Instance URL", "placeholder": "https://acme.service-now.com"},
                    {"name": "username", "label": "Integration username"}],
         "secret_label": "Password or API key"},
        {"connector_id": "azure_devops", "provider_type": "itsm", "display_name": "Azure DevOps",
         "fields": [{"name": "organization", "label": "Organization", "placeholder": "acme-corp"}],
         "secret_label": "Personal access token"},
        {"connector_id": "prometheus", "provider_type": "monitoring", "display_name": "Prometheus",
         "fields": [{"name": "base_url", "label": "Server URL", "placeholder": "http://prometheus:9090"}],
         "secret_label": "Bearer token (optional)", "secret_optional": True},
        {"connector_id": "grafana", "provider_type": "monitoring", "display_name": "Grafana",
         "fields": [{"name": "base_url", "label": "Server URL", "placeholder": "http://grafana:3000"}],
         "secret_label": "Service account token"},
        {"connector_id": "datadog", "provider_type": "monitoring", "display_name": "Datadog",
         "fields": [{"name": "site", "label": "Site", "placeholder": "datadoghq.com"},
                    {"name": "app_key_secret_ref", "label": "Application key reference", "placeholder": "env://DATADOG_APP_KEY"}],
         "secret_label": "API key"},
        {"connector_id": "dynatrace", "provider_type": "monitoring", "display_name": "Dynatrace",
         "fields": [{"name": "base_url", "label": "Environment URL", "placeholder": "https://abc12345.live.dynatrace.com"}],
         "secret_label": "API token"},
    ]}
...
async def validate_connection(
    ...
    async with _repo() as repo:
        ...
        try:
            connector_id = str(connection.discovery_scope.get("connector_id") or "")
            result = await _connector(connection.provider_type, connector_id=connector_id).validate_connection(connection)
        except NotImplementedError as exc:
...
async def discover_resources(
    ...
    async with _repo() as repo:
        ...
        try:
            connector_id = str(connection.discovery_scope.get("connector_id") or "")
            connector = _connector(connection.provider_type, connector_id=connector_id)
...
@app.get("/connections/{connection_id}/knowledge-candidates")
async def knowledge_candidates(
    connection_id: UUID,
    tenant_id: str = Query(min_length=1, max_length=128),
    limit: int = Query(default=20, ge=1, le=100),
) -> dict[str, Any]:
    """Preview knowledge-base candidates (resolved tickets/articles) from a ticketing
    connection. This is read-only: nothing is written to the knowledge base here. The
    caller reviews the returned items and submits the ones it wants through the
    governed /rag/knowledge-drafts approval workflow."""
    tenant_id = require_tenant_id(tenant_id, source="cloud operations knowledge candidates")
    async with _repo() as repo:
        row = await repo.get_connection(connection_id, tenant_id=tenant_id)
        if row is None:
            raise HTTPException(status_code=404, detail="Connection not found")
        if row.status != "validated":
            raise HTTPException(status_code=409, detail="Connection must be validated first")
        connection = CloudConnection.model_validate(repo.connection_payload(row))
    connector_id = str(connection.discovery_scope.get("connector_id") or "")
    factory = _ITSM_KNOWLEDGE_CONNECTORS.get(connector_id)
    if connection.provider_type != ProviderType.ITSM or factory is None:
        raise HTTPException(status_code=409, detail="Knowledge candidates are only available for ITSM connectors (jira, servicenow, azure_devops)")
    try:
        candidates = await factory().knowledge_candidates(connection, limit=limit)
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"Unable to fetch knowledge candidates: {exc}") from exc
