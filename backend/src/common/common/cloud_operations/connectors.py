...
_ITSM_CONNECTORS: dict[str, Callable[[], CloudConnector]] = {
    "jira": JiraConnector,
    "servicenow": ServiceNowConnector,
    "azure_devops": AzureDevOpsConnector,
}

_MONITORING_CONNECTORS: dict[str, Callable[[], CloudConnector]] = {
...
def connector_for(provider: ProviderType, *, connector_id: str = "", **options: Any) -> CloudConnector:
    ...
    if provider == ProviderType.ITSM:
        factory = _ITSM_CONNECTORS.get(connector_id)
        if factory is None:
            raise NotImplementedError(
                f"ITSM connector_id '{connector_id}' is not registered; choose one of {sorted(_ITSM_CONNECTORS)}"
            )
        return factory()
    if provider == ProviderType.MONITORING:
        factory = _MONITORING_CONNECTORS.get(connector_id)
        if factory is None:
            raise NotImplementedError(
                f"Monitoring connector_id '{connector_id}' is not registered; choose one of {sorted(_MONITORING_CONNECTORS)}"
            )
        return factory()
