...
class CloudConnectionBase(BaseModel):
    ...
    def credential_reference_required_for_real_provider(self) -> CloudConnectionBase:
        # Self-hosted Prometheus is commonly deployed unauthenticated behind network
        # policy, so it is the one connector allowed to omit credential_ref; every
        # other non-simulator provider (including every other monitoring/ITSM
        # connector) still fails closed without one.
        connector_id = str(self.discovery_scope.get("connector_id") or "")
        credential_optional = connector_id == "prometheus"
        if self.provider_type != ProviderType.SIMULATOR and not credential_optional and not self.credential_ref.strip():
