...
class IncidentEvidence(BaseModel):
    ...
    @classmethod
    def normalize_timestamps(cls, value: list[datetime]) -> list[datetime]:
        # Evidence rows arrive from many connectors; some stamp naive
        # datetimes (no tzinfo) and others aware ones. `FailurePatternAnalyzer
        # .analyze()` sorts every row's timestamps together
        # (`sorted(ts for row in rows for ts in row.timestamps)`), and Python
        # refuses to compare a naive datetime against an aware one - reproduced
        # live as a 500 on every `/incidents/bootstrap` call whose evidence mixed
        # the two, permanently blocking resolution-catalog bootstrap (and the
        # governed plan persistence that depends on it) for every incident.
        # Naive timestamps are assumed UTC, matching this codebase's `utc_now()`
        # convention everywhere else a bare datetime is produced.
        return [ts if ts.tzinfo is not None else ts.replace(tzinfo=UTC) for ts in value]
