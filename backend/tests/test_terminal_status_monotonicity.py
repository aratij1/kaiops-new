...
@pytest.mark.asyncio
async def test_rca_ready_outranks_investigating_on_a_timestamp_tie(sqlite_session_factory) -> None:
    """Real, live-reproduced gap: _STATUS_PRECEDENCE never listed "rca_ready"
    at all (a value resolution-agent's _resolution_projection_status now
    emits for a genuinely grounded, conclusive RCA with no ready-to-execute
    plan yet), so it ranked the same as "unknown" -- a stale "investigating"
    event arriving at the same timestamp as a genuine "rca_ready" event
    could win the tie-break and regress the projection back down."""
    incident_id = uuid4()
    tied_at = "2026-09-06T09:00:00+00:00"

    async with sqlite_session_factory() as session:
        repo = IncidentRepository(session)
        rca_ready_event = _event(incident_id=str(incident_id), status="rca_ready", event_type="incident.recommendation.generated")
        rca_ready_event["produced_at"] = tied_at
        await repo.save_incident_event(rca_ready_event)
        stale_investigating_event = _event(incident_id=str(incident_id), status="investigating", event_type="incident.context.collected")
        stale_investigating_event["produced_at"] = tied_at
        await repo.save_incident_event(stale_investigating_event)
        await session.commit()
...
@pytest.mark.asyncio
async def test_rca_ready_genuinely_advances_past_investigating(sqlite_session_factory) -> None:
    incident_id = uuid4()
    async with sqlite_session_factory() as session:
        repo = IncidentRepository(session)
        await repo.save_incident_event(_event(
            incident_id=str(incident_id), status="investigating", event_type="incident.alert.enriched",
        ))
        await repo.save_incident_event(_event(
            incident_id=str(incident_id), status="rca_ready", event_type="incident.recommendation.generated",
        ))
        await session.commit()
