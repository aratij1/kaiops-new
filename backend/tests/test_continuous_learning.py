...
def test_analyze_tolerates_naive_and_aware_timestamps_in_the_same_batch() -> None:
    """Evidence connectors are not consistent about tzinfo: some stamp naive
    datetimes, others aware ones. `analyze()` sorts every row's timestamps
    together, and Python raises TypeError comparing a naive datetime against
    an aware one - reproduced live as a 500 on every real
    `/incidents/bootstrap` call whose evidence batch mixed the two,
    permanently blocking resolution-catalog bootstrap (and the governed plan
    persistence that depends on it) for every incident in the platform.
    """
    naive = datetime(2026, 9, 6, 5, 0, 0)
    aware = datetime(2026, 9, 6, 7, 0, 0, tzinfo=UTC)
    evidence = IncidentEvidence(
        incident_id="i-1", service="checkout", environment="prod", alert_type="http-500",
        symptoms=["checkout HTTP 500"], timestamps=[naive, aware], affected_components=["api"],
    )
    assert all(ts.tzinfo is not None for ts in evidence.timestamps)
    patterns = FailurePatternAnalyzer().analyze([evidence])
    assert patterns[0].occurrence_frequency == 1
