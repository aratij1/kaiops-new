...
@pytest.mark.asyncio
async def test_cancelled_disconnect_does_not_trip_breaker_but_real_disconnect_does():
    from types import SimpleNamespace
    engine = create_async_engine("sqlite+aiosqlite:///:memory:")
    breaker = CircuitBreaker(failure_threshold=1, recovery_seconds=60)
    install_db_circuit_breaker(engine, breaker)
    handler = list(engine.sync_engine.dialect.dispatch.handle_error)[-1]
    handler(SimpleNamespace(is_disconnect=True, original_exception=asyncio.CancelledError()))
    assert breaker.allow()
    handler(SimpleNamespace(is_disconnect=True, original_exception=ConnectionError("lost database")))
