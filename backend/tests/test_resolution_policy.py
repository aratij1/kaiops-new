...
def test_hotl_is_disabled_by_default_even_for_eligible_action() -> None:
...
def test_hotl_stays_disabled_without_an_explicit_deployment_override_even_when_fully_eligible() -> None:
    # Formerly named test_environment_variable_cannot_enable_hotl_in_p0_safety_mode:
    # the platform's original P0 stance was that no env var alone was sufficient
    # governance to enable autonomous mutation. A deployment operator has since
    # made a deliberate, explicit decision to accept that tradeoff for this
    # deployment (auto_execution_enabled) - this test still proves the default
    # (the parameter omitted, exactly as every existing caller does today)
    # remains hitl-only, unconditionally, for a fully gate-clearing action.
    decision = evaluate_resolution_policy(_input(risk="low", confidence=0.95))
    assert decision.decision == "hitl"
    assert "hotl_disabled_p0_safety_mode" in decision.reason_codes


def test_deployment_override_only_grants_hotl_once_every_other_gate_already_passed() -> None:
    decision = evaluate_resolution_policy(_input(risk="low", confidence=0.95), auto_execution_enabled=True)
    assert decision.decision == "hotl"
    assert decision.required_approver_role == "policy-engine"
    assert "deployment_auto_execution_override" in decision.reason_codes
    assert "hotl_disabled_p0_safety_mode" not in decision.reason_codes


def test_deployment_override_does_not_relax_any_individual_gate() -> None:
    # The override only changes what happens once hitl_reasons is already
    # empty - it must never paper over a real failing gate.
    base = {"risk": "low", "confidence": 0.95}
    for overrides in (
        {"risk": "high"},
        {"confidence": 0.80},
        {"reversible": False},
        ...
        {"environment": "prod", "database_change": True},
    ):
        decision = evaluate_resolution_policy(_input(**{**base, **overrides}), auto_execution_enabled=True)
        assert decision.decision == "hitl", overrides
        assert "deployment_auto_execution_override" not in decision.reason_codes, overrides
