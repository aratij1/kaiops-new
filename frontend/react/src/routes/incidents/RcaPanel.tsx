...
/**
 * A conclusive, conflict-free, evidence-backed RCA that is still short of the
 * full auto-eligible bar (confidence/grounding/declared-gap checks) is worth
 * surfacing to an operator - instead of sitting indefinitely blocked - once
 * evidence collection has genuinely run its course for the remaining gaps
 * (nothing left in flight, nothing left to wait for). This never changes what
 * is eligible for governed auto-execution, only whether a human gets a
 * labeled, partial-evidence recommendation to weigh instead of nothing.
 */
export function isOperatorReviewEligible(input: {
  reviewRequired: boolean;
  investigationConclusive: boolean;
  conflictCount: number;
  integrityVerified: boolean;
  evidenceCount: number;
  citationCount: number;
  evidenceCollectionExhausted: boolean;
}): boolean {
  return Boolean(
    input.reviewRequired
    && input.investigationConclusive
    && input.conflictCount === 0
    && input.integrityVerified === true
    && input.evidenceCount > 0
    && input.citationCount > 0
    && input.evidenceCollectionExhausted,
  );
}
...
export default function RcaPanel({
  ...
  // Evidence gaps and thin grounding/confidence do not always mean the investigation
  // stalled - for a service that structurally can't produce richer evidence (no
  // tracing, no approved runbook), waiting longer never closes those gaps. When the
  // backend confirms nothing is still in flight (every declared gap has either been
  // collected or dead-lettered/exhausted), a conclusive, conflict-free, evidence-backed
  // RCA is worth a human's judgment instead of sitting blocked indefinitely. This never
  // relaxes what is eligible for governed auto-execution - that stays gated by `reviewRequired`
  // (and, further downstream, the backend-signed approval-readiness receipt) exactly as before.
  const evidenceCollectionExhausted = Boolean(canonicalEvidenceReadModel?.evidence_collection_exhausted);
  const operatorReviewEligible = isOperatorReviewEligible({
    reviewRequired,
    investigationConclusive,
    conflictCount: conflictingEvidence.length,
    integrityVerified: selectedAiTrust?.integrityVerified === true,
    evidenceCount: evidenceRows.length,
    citationCount,
    evidenceCollectionExhausted,
  });
  const decisionStatus = !reviewRequired
    ? "Investigation conclusive"
    : operatorReviewEligible
      ? "Partial evidence — review required"
      : "Review required";
    ...
    <section className="combined-analysis-page context-workspace">
      ...
      {canonicalWorkspace ? <header className="context-workspace-hero canonical-investigation-hero">
        ...
        <div className="context-workspace-summary" aria-label="Analysis summary">
          <span className={`context-status is-${reviewRequired ? (operatorReviewEligible ? "partial" : "review") : "ready"}`}>
      ...
      {canonicalWorkspace && canonicalWorkspace.resolution?.status !== "ready" ? <p className="ai-trust-warning" role="status"><ShieldAlert size={16} />Context collection is complete, but this incident has no RCA-bound evidence yet. Replace the hypothesis with a verified causal observation and cite it before a governed resolution plan can be published.</p> : null}
      ...
      <section className="investigation-scoreboard" aria-label="Investigation scores and evidence summary">
        ...
        <article><span>Citation coverage</span><strong>{groundingScore === null ? "Unavailable" : formatQualityPercent(groundingScore)}</strong><small>Accepted citations, not causal confirmation</small></article>
      ...
      {rcaDetailView === "simple" ? <details className="investigation-section investigation-attention" open={reviewRequired}>
        ...
        <div className="investigation-section-body"><DecisionReadinessPanel title="Investigation readiness" checks={investigationChecks} eligibleLabel="Evidence ready for operator review" partiallyEligible={operatorReviewEligible} partiallyEligibleLabel="Partial evidence — operator review recommended" onReviewEvidence={() => { onSetRcaAnalysisMode("fresh"); void onRerunRca("fresh"); setReviewEvidenceRequest((value) => value + 1); }} />
      ...
      <section className="analysis-workbench" aria-labelledby="analysis-workbench-title">
        ...
        <div className={`analysis-alert ${reviewRequired ? (operatorReviewEligible ? "is-partial" : "needs-review") : "is-ready"}`} role="status">{reviewRequired ? <ShieldAlert size={18} /> : <CheckCircle2 size={18} />}<div><strong>{decisionStatus}</strong><span>{operatorReviewEligible ? "Evidence collection is exhausted for the remaining gaps - review this conclusion with judgment before acting." : missingEvidence.length ? `Missing evidence: ${missingEvidence.join(", ")}` : "The current evidence package supports a guarded operator decision."}</span></div></div>
        ...
        <footer className="rca-analysis-meta"><span>Analysis: <strong>{String(selectedRcaDecision?.status || "unknown").replaceAll("-", " ")}</strong></span><span>Latest evidence: <strong>{evidenceRows[0]?.timestamp ? formatUtcTimestamp(evidenceRows[0].timestamp) : "timestamp not supplied"}</strong></span><span>Citation coverage: <strong>{groundingScore === null ? "Unavailable" : formatQualityPercent(groundingScore)}</strong></span><span>Citations: <strong>{citationCount}</strong></span></footer>
