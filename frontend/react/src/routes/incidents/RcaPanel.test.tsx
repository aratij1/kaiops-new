...
describe("RcaPanel canonical evidence gate", () => {
  ...
  const conclusiveExhaustedInputs = {
    reviewRequired: true, investigationConclusive: true, conflictCount: 0,
    integrityVerified: true, evidenceCount: 3, citationCount: 2, evidenceCollectionExhausted: true,
  };
  it("offers operator review when evidence collection is exhausted but the full bar isn't cleared", () => {
    expect(isOperatorReviewEligible(conclusiveExhaustedInputs)).toBe(true);
  });
  it("never offers operator review while evidence collection is still in flight", () => {
    expect(isOperatorReviewEligible({ ...conclusiveExhaustedInputs, evidenceCollectionExhausted: false })).toBe(false);
  });
  it("never offers operator review with an inconclusive investigation, even if collection is exhausted", () => {
    expect(isOperatorReviewEligible({ ...conclusiveExhaustedInputs, investigationConclusive: false })).toBe(false);
  });
  it("never offers operator review while conflicting evidence is unresolved", () => {
    expect(isOperatorReviewEligible({ ...conclusiveExhaustedInputs, conflictCount: 1 })).toBe(false);
  });
  it("never offers operator review without a verified evidence-integrity contract", () => {
    expect(isOperatorReviewEligible({ ...conclusiveExhaustedInputs, integrityVerified: false })).toBe(false);
  });
  it("never offers operator review on zero bound evidence or zero traceable citations", () => {
    expect(isOperatorReviewEligible({ ...conclusiveExhaustedInputs, evidenceCount: 0 })).toBe(false);
    expect(isOperatorReviewEligible({ ...conclusiveExhaustedInputs, citationCount: 0 })).toBe(false);
  });
  it("has nothing to offer when the full bar is already cleared", () => {
    expect(isOperatorReviewEligible({ ...conclusiveExhaustedInputs, reviewRequired: false })).toBe(false);
  });
