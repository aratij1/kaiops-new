...
export type ConnectorField = { name: string; label: string; placeholder?: string };
export type ConnectorCatalogEntry = {
  connector_id: string;
  provider_type: "itsm" | "monitoring";
  display_name: string;
...
export function listConnectorCatalog(accessToken: string) {
  return requestJson<{ connectors: ConnectorCatalogEntry[] }>(accessToken, "/cloud-ops/connectors/catalog").then((data) => data.connectors);
}

export function createIntegrationConnection(accessToken: string, input: {
  projectId: string;
  connectorId: string;
  providerType: "itsm" | "monitoring";
  ...
  return requestJson<ConnectionResponse>(accessToken, "/cloud-ops/connections", {
    ...
    body: JSON.stringify({
      ...
      credential_ref: input.secretRef,
      connection_owner: input.owner,
      read_capability: true,
      write_capability: false,
      allowed_regions: ["global"],
      resource_filters: {},
      discovery_scope: { connector_id: input.connectorId, ...input.scopeFields },
    }),
  }).then((data) => data.connection);
}

export function fetchKnowledgeCandidates(accessToken: string, connectionId: string, limit = 20, signal?: AbortSignal) {
  return requestJson<{ candidates: KnowledgeCandidate[]; count: number }>(
    accessToken,
    `/cloud-ops/connections/${encodeURIComponent(connectionId)}/knowledge-candidates?limit=${limit}`,
    { signal },
  ).then((data) => data.candidates);
}

export function submitKnowledgeDraft(accessToken: string, candidate: KnowledgeCandidate) {
  return requestJson<{ id?: string }>(accessToken, "/rag/knowledge-drafts", {
    method: "POST",
    body: JSON.stringify({
      kind: "runbook",
