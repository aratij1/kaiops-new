...
/** Onboarding is a pipeline (connect -> validate -> discover -> knowledge base), so
 * show a connection's actual position in it at a glance instead of two undifferentiated
 * buttons. Knowledge base only applies to ticketing connections. */
function ConnectionPipeline({ connection }: { connection: CloudConnection }) {
  const isItsm = connection.provider_type === "itsm";
  const steps = [
    { key: "connect", label: "Connected", done: true },
    { key: "validate", label: "Validated", done: connection.status === "validated" },
    { key: "discover", label: "Discovered", done: Boolean(connection.last_discovery_at) },
    ...(isItsm ? [{ key: "knowledge", label: "Knowledge base", done: false }] : []),
  ];
  return (
    <ol className="cloud-ops-pipeline" aria-label="Onboarding progress">
      {steps.map((step, index) => (
        <li key={step.key} className={step.done ? "is-done" : ""}>
          <span className="cloud-ops-pipeline-marker">{step.done ? <Check size={11} /> : index + 1}</span>
...
export default function CloudConnectionsRoute() {
  ...
  return (
    <section className="cloud-ops-route" aria-labelledby="cloud-connections-title">
      <ConnectToolsPanel projectId={normalizedProjectId} owner="cloud-ops-admin-ui" onConnected={() => void refresh()} />
          ...
          <article className="cloud-ops-card" key={connection.id}>
            <header>
              <div>
                ...
                <p>{connection.project_id} · {String(connection.discovery_scope?.connector_id || connection.provider_type)}</p>
              </div>
            ...
            {connection.status === "failed" && connection.failure_reason ? <p className="cloud-ops-error" role="alert">{connection.failure_reason}</p> : null}
            <ConnectionPipeline connection={connection} />
            <div className="cloud-ops-toolbar">
              ...
              {connection.last_discovery_at ? (
                <Link className="button-secondary" to="/cloud-ops/resources">
                  View discovered resources
                </Link>
              ) : null}
            </div>
            {connection.provider_type === "itsm" && connection.status === "validated" ? (
              <KnowledgeCandidatesPanel connectionId={connection.id} />
            ) : null}
          </article>
        ))}
      </div>
