...
export default function ApprovalsRoute() {
  ...
  return <section className="grid single-col approval-workspace">
    ...
    {view === "queue" ? <article className="panel">
      ...
      {(packet.row.assignee || packet.row.assignment_status || packet.row.jira_issue_key) ? <section className="approval-assignment" aria-label="Human-in-the-loop assignment">
        <div><small>Assigned reviewer</small><strong>{textValue(packet.row.assignee)}</strong></div>
        <div>
          <small>Assignment status</small>
          <span
            className={`pill status-${packet.row.jira_sync_failed ? "failed" : String(packet.row.assignment_status || "").toLowerCase() === "assigned" ? "success" : "review"}`}
            title={textValue(packet.row.assignment_reason)}
          >
            {textValue(packet.row.assignment_status)}
          </span>
        </div>
        <div>
          <small>Jira ticket</small>
          {packet.row.jira_issue_key ? (
            packet.row.jira_issue_url
              ? <a href={String(packet.row.jira_issue_url)} target="_blank" rel="noreferrer noopener"><strong>{textValue(packet.row.jira_issue_key)}</strong></a>
              : <strong>{textValue(packet.row.jira_issue_key)}</strong>
          ) : <strong>Not linked</strong>}
        </div>
        {packet.row.jira_sync_failed ? <div className="approval-assignment-warning">
          <span className="pill status-failed" title={textValue(packet.row.assignment_reason)}>Jira sync failed</span>
          <p>{textValue(packet.row.assignment_reason)}</p>
        </div> : null}
      </section> : null}
