...
try {
  ...
  let olderPages=0;
  while(await page.getByRole("button",{name:"Load older closures",exact:true}).isVisible()) {
    if(++olderPages>10) throw new Error("Unexpected history pagination loop");
    const previous=await page.locator(".resolution-history-workspace tbody tr").count();
    await page.getByRole("button",{name:"Load older closures",exact:true}).click();
    await page.waitForFunction(n=>document.querySelectorAll(".resolution-history-workspace tbody tr").length>n,previous,{timeout:30000});
  }
  ...
  const result={historyRows:displayed.length,olderPages,batchClosuresVisible:found.length,blockersExpandedInitially:collapsed,inboxOverflowAt1100:overflow};
  writeFileSync("../../docs/architecture/current-kaims-closures-browser-2026-09-07.json",JSON.stringify(result,null,2));
