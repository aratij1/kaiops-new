...
async def diagnose(tenant_id: str, incident_id: str) -> list[tuple[str, str, str, str]]:
    ...
    params = {"tenant": tenant_id, "incident": UUID(incident_id).hex}
    # context_snapshots.incident_id is a plain String(128) column populated with
    # str(uuid) (dashed) everywhere it's written, unlike every UUID-typed column
    # used above (incidents.id, incident_investigation_bindings.incident_id, ...)
    # which SQLAlchemy's Uuid(as_uuid=True) normalizes to bare hex. Querying this
    # one table with the bare-hex incident_id silently matches zero rows even
    # when snapshots exist, which previously made this tool misreport a working
    # incident as stuck at "NO_SNAPSHOT".
    params_dashed = {"tenant": tenant_id, "incident": str(UUID(incident_id))}
    try:
        async with engine.connect() as connection:
            ...
            snapshot = await _one(
                connection,
                """
                SELECT snapshot_id, snapshot_version, snapshot_stage FROM context_snapshots
    ...
    return [
        ...
        (
            ...
            f"v{snapshot['snapshot_version']} ({snapshot['snapshot_stage']})" if snapshot else "-",
            "-" if snapshot else "NO_SNAPSHOT",
        ),
        (
            "Investigation",
            "PASS" if investigation and investigation["status"] in {"current", "grounded"} else "BLOCKED",
            f"RCA v{investigation['rca_version']}" if investigation else "-",
            "-" if investigation and investigation["status"] in {"current", "grounded"} else ("RCA_" + str(investigation["status"]).upper() if investigation else "NO_RCA"),
        ),
        (
            "Resolution",
