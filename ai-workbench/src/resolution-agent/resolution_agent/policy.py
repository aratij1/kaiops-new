...
def evaluate_resolution_policy(
    value: ResolutionPolicyInput, *, auto_execution_enabled: bool = False,
    ...
    # P0 safety mode was deliberately fail-closed: every gate above this point
    # (low risk, >=92% confidence, reversible, single-service blast radius,
    # canary-supported, no prod database mutation, approved+validated+rollback-
    # bound runbook, zero contradictions) already has to pass before reaching
    # here - "hotl_reasons" is empty precisely when every one of them cleared.
    # Deployment-operator override: an operator has explicitly decided, for
    # this deployment, that clearing every one of those gates is sufficient
    # governance to skip a human click - this does not lower or skip any gate
    # above, it only changes what happens once all of them already passed.
    if not hitl_reasons and auto_execution_enabled:
        return ResolutionPolicyDecision(
            decision="hotl",
            reason_codes=["all_low_risk_gates_cleared", "deployment_auto_execution_override"],
            required_approver_role="policy-engine",
        )
